...
#define DEFAULT_RECV_IP "239.28.0.101"
#define DEFAULT_RECV_PORT "9012"
#define DEFAULT_SEND_IP "239.28.0.100"
#define DEFAULT_SEND_PORT "9011"
...
int main(int argc, char** argv)
{
    ...
    while (keepRunning) {
        ...
        printf("INFO: Frame size: %u, manual: %u\n", frm_size, width * height * 3 / 2);
        ...
        if (strncmp(payload_type, "rtsp", sizeof(payload_type)) != 0) {
            if (dump_fp) {
                ...
                if (*(uint32_t *)ptr != frame_count) {
                    printf("Frame size: %u, manual: %u, len: %ld\n", frm_size, width * height * 3 / 2, buf->len);
