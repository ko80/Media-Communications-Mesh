...
//        INFO("rx_st20p_consume_frame rx_bufs: %p memif_conn: %p", rx_bufs, s->memif_conn);
    }
...
//            INFO("rx_st20p_consume_frame enter");
            rx_st20p_consume_frame(s, frame);
        }
//        INFO("st20p_rx_put_frame enter");
...
rx_session_context_t* mtl_st20p_rx_session_create(mtl_handle dev_handle, struct st20p_rx_ops* opts, memif_ops_t* memif_ops)
{
    ...
    rx_ctx->frame_size = st_frame_size(ST_FRAME_FMT_YUV422PLANAR10LE, ops_rx.width, ops_rx.height, false);
    ...
    // ops_rx.flags |= ST20P_RX_FLAG_PKT_CONVERT;
...
tx_session_context_t* mtl_st20p_tx_session_create(mtl_handle dev_handle, struct st20p_tx_ops* opts, memif_ops_t* memif_ops)
{
    ...
    tx_ctx->handle = tx_handle;
    tx_ctx->frame_size = st_frame_size(ST_FRAME_FMT_YUV422PLANAR10LE, ops_tx.width, ops_tx.height, false);
