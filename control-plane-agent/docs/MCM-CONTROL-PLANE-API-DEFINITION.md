# MCM Control Plane REST API Definition (`CP-API`)

* Mesh Agent runs a server to handle `CP-API` requests.
* `CP-API` is exposed to any external network configuration tools.
* `CP-API` is a REST API based on HTTP 1.1.
